    ...
    <Screen>
      ...
      <div className="sp-body" style={{ gap: 16 }}>
        ...
        <div className="sp-card" style={{ gap: 2 }}>
          <SecLabel kind="show">요청할 정보 (Required)</SecLabel>
          {allowed.map((c) => (
            <div key={c.label} className="sp-check">
        ...
        <div className="sp-card" style={{ gap: 2, background: "var(--gray-5)", border: "none" }}>
          <SecLabel kind="hide">요청 불가 · 민감정보</SecLabel>
          {blocked.map((c) => (
      ...
      <div className="sp-footer">
        <Link href="/landlord/results/req-001" style={{ textDecoration: "none" }}>
          <Btn variant="primary" iconR={<I.ArrowR size={18} stroke={2} />}>검증 요청 생성</Btn>
        </Link>
      </div>
      <HomeBar />
    </Screen>
